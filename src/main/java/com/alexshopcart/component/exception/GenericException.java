package com.alexshopcart.component.exception;
...
public class GenericException extends RuntimeException {
	...
	public GenericException() {
		...
		this.baseError = ErrorDto.builder().httpHeaders(new HttpHeaders()).httpStatus(HttpStatus.INTERNAL_SERVER_ERROR)
				.field("").build();
	}
	...
	public GenericException(HttpStatus httpStatus, HttpHeaders httpHeaders, String field, String message,
			List<String> errors) {
		super(message);
		this.baseError = ErrorDto.builder().httpHeaders(httpHeaders).httpStatus(httpStatus).field(field).errors(errors)
				.build();
	}
